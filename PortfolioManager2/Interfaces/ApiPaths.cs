namespace Interfaces
{
    public class ApiPaths
    {
        public const string BuyTransaction = "api/buytransaction";
        public const string CorporateAction = "api/corporateaction";

        public const string CashDeposit = "api/transactions/cashdeposit";
        public const string CashWithdrawal = "api/transactions/cashwithdrawal";
        public const string Accounts = "api/accounts";
        public const string Portfolios = "api/portfolios";
        public const string AccountTransactions = "api/transactionsummary";
        public const string Investments = "api/investments";
        public const string InvestmentMap = "api/accountinvestmentmap";
        
    }
}